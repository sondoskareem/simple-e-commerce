...
module.exports = function(path,app) {
	...
	app.route(`${path}/order/center`).get(check_center.check_center, Order.orderForCenter_id);

	app.route(`${path}/order/user`).get(check_user.check_user, Order.OrderForUser);
	app.route(`${path}/order/user`).get(check_user.check_user, Order.OrderForUser_id);

	app.route(`${path}/order/admin`).get(checkLogin_admin.checkLogin_admin, Order.orderForAdmin);
	app.route(`${path}/order/admin`).get(checkLogin_admin.checkLogin_admin, Order.orderForAdmin_id);
